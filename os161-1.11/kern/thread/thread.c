 ...
 * Destroy a thread.
 *
 * This function can not be called in the victim thread's own context.
	    ...
	    struct thread **ret)
{
    ...
    /* Allocate the new pid and process for this new thread */
    mypid = new_pid();
    if(mypid == -1) {
        kfree(newguy->t_name);
    ...
    curproc = array_getguy(processTable, mypid);
    newguy->pid = mypid;
    curproc->myThread = newguy;
    ...
    if(func == md_forkentry) {
        ...
        /* Clone the open file handles */
        make_copy(curthread, newguy);
        if (newguy->dt == NULL) {
            kfree(newguy->t_name);
            kfree(newguy->t_stack);
            kfree(newguy);
        ...
        curproc->parent_pid = curthread->pid;
        
        /* Copy the vmspace and stack */
        as_copy(curthread->t_vmspace, &(newguy->t_vmspace));

//        newguy->t_stack = curthread->t_stack;

    }else {

        result = constructor(newguy);
...
//#if OPT_A2
//
//    if(curthread->dt != NULL) {
//        int result = destructor(curthread->dt);
//    }
//#endif
...
/*
 * Wake up the first thread who are sleeping on "sleep address"
 * ADDR.
 */
void
thread_wakeup_first(const void *addr)
{
	int result;
    int found = 0;
    int i = 0;
