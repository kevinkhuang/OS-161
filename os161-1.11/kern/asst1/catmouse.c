...
static struct array *locks;     // Array of the locks of the bowls.
static struct cv *noCats;       // the noCats cv.
static struct cv *noMice;       // the noMice cv.
volatile int countCats;         // the counter of the number of the cats eating.
volatile int countMice;         // the counter of the number of the mice eating.
               ...
               unsigned long catnumber)
{
    ...
    for(i=0;i<NumLoops;i++) {
        ...
        struct lock *curbowl = array_getguy(locks, bowl - 1);
        lock_acquire(curbowl);

            while (countMice != 0) {
                cv_wait(noMice, curbowl);
            }
            countCats++;
         ...
         char ** args)
{
    ...
    locks = array_create();
    if (locks == NULL) {
        kprintf("Cannot create locks array\n");
    }
    int result = array_preallocate(locks,NumBowls);
    if (result) {
        kfree(locks);
    }
    for (j = 0; j < NumBowls; j++) {
        struct lock *bowlLock = lock_create("bowlLock");
        array_add(locks, bowlLock);
    }
    
    /* initialize the cv */
    noCats = cv_create("noCats");
    noMice = cv_create("noMice");
    ...
    /* clean up the locks that we created */
    for (j = 0; j < NumBowls; j++) {
        struct lock *bowlLock = array_getguy(locks, j);
        lock_destroy(bowlLock);
    }
    
    /* clean uo the cv that we created */
    cv_destroy(noCats);
    cv_destroy(noMice);
    
    /* clean uo the array that we created */
    array_destroy(locks);
